...
- **A `Get-Process` main window handle can be the console, not the game.** `tools/capture-window.ps1` captured a black console window because the run was launched *without* output redirection, so `game.exe` owned a console whose handle won. It also needs `AttachThreadInput` to reliably take the foreground: `SetForegroundWindow` alone silently fails from a background process, and since the capture reads screen pixels, a window that is not on top captures whatever is.
