...
- **The agent is the artist on this project.** Stated plainly by the user, 2026-07-30: _"buddy there is no artist, you ARE the artist. this is a solo dev project."_ Do not defer art decisions, do not ship deliberately rough placeholders and call them someone else's problem, and do not treat "I can't see the screen" as an excuse — magnify the output and look at it (`tools/preview-textures.ps1` exists for exactly this).
- **Third-party art may be used as reference and must never be shipped.** A Minecraft asset mirror was dropped into `assets/textures/blocks/` for reference; because the build copies `assets/` wholesale, that silently put 925 of Mojang's PNGs into the game's output folder. Reference material now lives in `reference/`, gitignored and outside `assets/`. The project's whole premise is original work, so this boundary is worth keeping mechanically enforced rather than relying on memory. **The HUD sprite sheet currently in `assets/textures/hud.png` is a known exception** — it is Minecraft's widget art, kept at the user's explicit request, and must be replaced before any release.
