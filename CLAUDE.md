...
**Read this before debugging anything.** Every one of these got past a clean build at `/W4` *and* a clean soak, and every one was found by the user rather than by a tool. What is here is enough to *recognise* one; **`LESSONS.md` has the full story of each**, and the fix is usually a `constexpr` assert rather than care.
...
2. **Widening a family predicate kills every early-out already sitting behind it.** `isFurnace` grew to cover smokers and silently turned eight `case` labels in `blockName` into dead code. **Grep every existing caller, not just the new one.**
