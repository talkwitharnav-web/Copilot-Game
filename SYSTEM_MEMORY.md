...
**Index** - 153 sections. Line numbers, so a reader can jump straight there.
**Generated by `tools/index-docs.ps1`; do not hand-edit. Re-run it after editing this file.**

- **1** &middot; SYSTEM_MEMORY.md
  - **176** &middot; Critical Invariants
  - **189** &middot; Verified Environment
    - **210** &middot; Two GPUs
    - **214** &middot; What hardware this actually needs
    - **225** &middot; Third-party Vulkan layers are present on this machine
  - **231** &middot; Repository Layout
  - **299** &middot; Build System
    - **301** &middot; Targets
    - **310** &middot; Presets
  - **332** &middot; Dependencies
    - **347** &middot; Dependency sources are nested Git repositories, and that is expected
  - **353** &middot; Engine Architecture
  - **387** &middot; Game Architecture (current)
  - **433** &middot; World Generation
    - **439** &middot; The five stages
    - **449** &middot; The climate fields
    - **465** &middot; Shaping
    - **485** &middot; Density and the lattice
    - **516** &middot; Surface rules
    - **559** &middot; Caves
    - **575** &middot; Ores
    - **607** &middot; Biomes
    - **621** &middot; Named divergences
    - **633** &middot; Measuring it
  - **655** &middot; Frame Pacing
  - **667** &middot; Shaders
  - **673** &middot; Rendering Approach
    - **683** &middot; What a frame is made of
    - **704** &middot; Winding and culling — do not "fix" this by reasoning
    - **708** &middot; Camera and projection
  - **716** &middot; Controls
    - **758** &middot; Rumble
  - **861** &middot; Player Physics
    - **894** &middot; Water
  - **996** &middot; Block Textures
  - **1015** &middot; Creatures
    - **1021** &middot; Creature skins
    - **1051** &middot; Reviewing a model
    - **1097** &middot; The species table
    - **1129** &middot; How a population arrives, and what happens to it
    - **1175** &middot; How a creature decides what to do
    - **1200** &middot; A villager's day
    - **1224** &middot; The iron golem
    - **1237** &middot; Fighting at a distance, and what a creature holds
    - **1255** &middot; What it takes to be noticed, and what it takes to be forgotten
    - **1271** &middot; Closing in, and the blow itself
    - **1298** &middot; Dying
    - **1333** &middot; What a Bramble's blast actually does
    - **1347** &middot; How limbs move
    - **1365** &middot; Finding a way round
    - **1393** &middot; Getting up a hill
    - **1411** &middot; Spawn eggs
    - **1423** &middot; Creature sizes
  - **1471** &middot; Workstations and containers
    - **1475** &middot; The smoker
    - **1483** &middot; The smithing table
    - **1491** &middot; The chest, and the four families that are one
    - **1527** &middot; The stowbox
    - **1541** &middot; The hopper
    - **1551** &middot; The stonecutter
    - **1557** &middot; The bell, the cauldron and the lectern
  - **1573** &middot; Blocks: ids, shapes and authoring
    - **1577** &middot; The id space
    - **1591** &middot; Adding a block type
    - **1625** &middot; The four box sets, and which one is the drawn shape
    - **1640** &middot; Breaking cracks
    - **1657** &middot; What the ground-cover pass places
    - **1681** &middot; Authoring
  - **1699** &middot; Items, crafting, smelting and mining
    - **1703** &middot; Items and drops
    - **1759** &middot; Crafting
    - **1775** &middot; The crafting table
    - **1783** &middot; The furnace
    - **1818** &middot; Mining and tools
  - **1857** &middot; Torches
  - **1867** &middot; Staying Alive
  - **1898** &middot; Effects, potions and brewing
  - **1936** &middot; Sound
  - **1967** &middot; HUD
    - **1979** &middot; The sprite sheet
    - **1989** &middot; Text
    - **2001** &middot; Hotbar
...
*Split out of `## Block Textures` on 2026-08-19. Everything from here to `## Workstations
and containers` had accumulated under that heading, which is why a creature question used
to mean reading a texture section. Nothing moved — only the heading above it.*
...
> ⛔ **A creature that walks out of range is RETIRED, and until 2026-08-19 that included villagers and the player's own golems.** The test is distance-based and takes anything past
> `m_activeRadius`, floored at `kBaseRadius = 90.0f` — about five chunks, so **walking to a nearby
> cave was enough.** Retirement is silent by design: no loot, no particle, no log line, because only
> a *kill* leaves anything behind. And because `m_populated` already holds the column, **the village
> was never repopulated** — the next autosave wrote the roster without them, so the loss reached disk.
>
> **What made it worse than an ordinary despawn is what went with them:** a villager's `profession`,
> `bedCell`, `jobCell` and `meetCell` — the four fields a save-format bump had been made specifically
> to persist. **The save layer was carrying state the tick loop deleted before it could ever be
> reloaded**, and `playerBuilt`'s own comment already said *"This one has to be saved."*
>
> **Now `species.keepsHouse || creature.playerBuilt` exempts a creature from the DISTANCE arm only.**
> A dead one, and one that has fallen out of the world, still retire. Both flags already existed and
> were already on disk, so the fix is one clause rather than a format change.
...
*Split out of `## Block Textures` on 2026-08-19.*
...
## Blocks: ids, shapes and authoring
...
## Items, crafting, smelting and mining
...
**A drop nobody collects is gone after five minutes.** Without that a long session accumulates every item it ever made, and each one rebuilds its geometry every frame. The reference pauses the timer when a chunk stops ticking; ours runs on wall time, which is simpler. **The justification for that simplification changed on 2026-08-19 and the conclusion survived, but only just — do not re-derive it from the old premise.** This paragraph used to rest on drops being absent from disk entirely; they are now saved. *(The superseded clause is described rather than quoted, deliberately — a retired negative left spelled out is a permanent false positive for the next sweep, which `ItemEntity.cpp` records having paid for the same day.)* What keeps wall time defensible is instead that `age` is **persisted and restored verbatim**: `SavedItem::age` round-trips through `loadDrops`, and the restore goes through `ItemEntities::restore` rather than `spawn` precisely so the clock is not reset. Had it gone through `spawn`, quitting and reloading would have set every drop's age back to zero and a reload would have been a way to keep a stack for ever. **The remaining inequivalence is real and is a deliberate accepted cost:** wall time keeps running while the game is closed, so a drop left overnight is gone on the next launch, where the reference would still have it. *Re-check by reading `age` and `pickupDelay` through `SavedItem` and `ItemEntities::restore`; this becomes wrong if either stops being carried.*
...
**Amethyst is not an ore row and is not placed this way.** Geodes generate as a **structure**, added
2026-08-19: a hollow shell with an outer skin, budding blocks and four cluster growth stages, placed
into a band whose limits are **derived rather than written** — `kLavaLevel + kGeodeReach + 1` at the
bottom and `medianSurfaceHeight() − kGeodeReach` at the top, so a change to the lava floor or the
surface CDF moves the band instead of silently stranding it. `kGeodeSalt` is checked against every
`kOreVeins` salt by `static_assert`, because two features sharing a salt would place in lockstep.

**This closed the last two dead overworld recipes.** The spyglass and the amethyst block had ids,
recipes and cluster stages for milestones, and **nothing placed the source anywhere in worldgen** —
`AmethystCluster`/`Amethyst`/`BuddingAmethyst` measured 0/0/0 across five files. **A drop arm whose
block nothing places is a circle, not a route**, and no recipe matcher can see it.
...
**`drop_showcase=1` throws one of every awkward block on the floor** a few paces north of the spawn point, with the sprite-path drops beside them for comparison. **The world's own dropped items are neither loaded nor saved while it is on**, gated in `Main.cpp` on `!settings.dropShowcase` at both the `loadDrops` restore and the `saveDrops` write — the same shape `creature_showcase` uses. **Neither gate is decoration:** remove the save gate and every model review writes the showcase roster, anvil and enchanting table included, into the real world file. *(This line asserted the opposite until 2026-08-19 — that drops never reached disk, which made it an argument for deleting two live gates. The retired sentence is described rather than quoted, deliberately: a superseded negative left spelled out is a permanent false positive for the next sweep, as `ItemEntity.cpp` records having paid for the same day. Re-check with a search of `Main.cpp` for `saveDrops|loadDrops`.)*
...
> ⛔ **Sway is packed into FOUR BITS, so the tallest plants saturated and bent half as far as they
> should — silently, from the eighth block up, until 2026-08-19.** The cross-block walk that gives a
> tall plant its gradient was bounded by a hand-written `7` under a comment that did not match it, so
> the root reached 14 and a top corner added `+2` = **16 against a maximum of 15**. `packVertexSurface`
> **saturates**, so top and bottom decoded 15 and 14 — **a gradient of 1 where every other plant gets
> 2**. Every kelp, bamboo, cave-vine and twisting or weeping vine above eight blocks was affected, on a
> green build with no warning: `Vertex.hpp`'s asserts **compare the packer against itself** and cannot
> see the mesher's emitted range at all.
>
> **The bound is now discovered rather than written.** `swayFieldMax()` walks `v` until
> `pack(v) == pack(v+1)`, so it finds the field's maximum **with zero knowledge of the bit layout** —
> widen the field and the walk widens itself. Verified by simulation at 4, 5 and 6 bits.
>
> ⚠️ **Bamboo (max 16) and full-height kelp now SHARE the top sway value above seven blocks.** They no
> longer bend at half strength, but they stop differentiating. Fixing that needs the field widened into
> the free bits 16–31, and **both GLSL decoders re-spell the bit positions as bare literals with
> nothing binding them to `Vertex.hpp`** — so it is a three-file change that `glslc` must see, not a
> constant edit.
