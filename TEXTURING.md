...
- **1** &middot; Reading Reference Textures, and Authoring Our Own
  - **66** &middot; 1. What a box net is
    - **83** &middot; Deriving `w`, `h`, `d` from the image
  - **108** &middot; 2. The measurements worth taking
    - **112** &middot; Alpha map — gives you the net dimensions
    - **123** &middot; Palette and luminance spread — tells you how to author yours
    - **141** &middot; Locating a feature — settles "where is the face?"
    - **152** &middot; Region dump — shows you the anatomy
    - **165** &middot; Crop a region out to look at it
  - **171** &middot; 3. Traps, all of which cost real time
  - **199** &middot; 4. Authoring ours
    - **201** &middot; What is ours and what is shared
    - **211** &middot; Palette discipline is a property of the subject, not a house style
    - **231** &middot; Dithering technique
    - **243** &middot; Grain: where a 25-colour subject's colours actually are
    - **257** &middot; Markings are shapes, not per-pixel odds
    - **270** &middot; When a first attempt is wrong, rewrite it — do not tweak it
    - **284** &middot; Shading can be per face rather than per pixel
    - **302** &middot; The generator pattern
  - **312** &middot; 5. Checking it mathematically
    - **338** &middot; Two traps in the measurement itself
  - **348** &middot; 6. Verification checklist
  - **362** &middot; 7. PowerShell pitfalls specific to this work
  - **386** &middot; 8. Worked example: the sheep
  - **408** &middot; 9. Worked example: the Bramble
  - **422** &middot; 10. Worked example: the cow
  - **438** &middot; 11. Worked example: the pig
  - **455** &middot; 12. Proving a model before authoring its skin
    - **469** &middot; Read the alpha as runs, not as a picture
  - **486** &middot; 13. The 26.2 roster, measured
    - **780** &middot; The aquatic roster, measured (M20i)
    - **895** &middot; A villager-family overlay slot we do not build
    - **899** &middot; Reading a net you have never seen before
    - **923** &middot; A net is not a pose
    - **947** &middot; Scale is per species, not per net
  - **957** &middot; 14. Formula reference
    - **962** &middot; 14.1 Net arithmetic
    - **990** &middot; 14.2 Box extents in world units
    - **1074** &middot; 14.3 Vertical reach of a pitched box
    - **1087** &middot; 14.4 Choosing `modelScale`
    - **1095** &middot; 14.5 The z-fighting check — run this before every render
    - **1117** &middot; 14.5b When the reference itself puts two faces on one plane
    - **1147** &middot; 14.5c Two boxes that merely *touch* have the same problem
    - **1167** &middot; 14.6 Hop physics (species with `hops`)
    - **1193** &middot; 14.7 Pixel-count checks
...
**A *block* texture can be a net too, and the empty rows are the clue.** `red_bed_foot_side.png` is not a face: rows 0–7 are transparent, 7–13 are the mattress and its frame, and 13–16 hold **three 3×3 leg faces packed side by side**. Sampling it as a plain 16×16 face from row 0 drew mostly nothing and no legs at all. Everything §1 says about creature nets applies here — **dump the pixels before choosing a rectangle**, and `tools/dump-pixels.ps1` answers it in one command.

**A model's own geometry is in the repo, so read it rather than guessing.** `reference/minecraft-assets-26.2/.../models/block/*.json` gives every box's `from`, `to` and per-face `uv` verbatim. The bell's stone posts and plank bar, the grindstone's five boxes and the brewing stand's three base plates all came straight off it. A guess costs a build and a render each time; reading costs one command.
