...
**Index** - 42 sections. Line numbers, so a reader can jump straight there.
**Generated by `tools/index-docs.ps1`; do not hand-edit. Re-run it after editing this file.**

- **1** &middot; DECISIONS.md
  - **69** &middot; Bedrock Edition is the reference, not Java
  - **86** &middot; C++20 from day one, no prototype-in-another-language phase
  - **90** &middot; Vulkan, not DirectX 12 or OpenGL
  - **98** &middot; GLFW for windowing, not raw Win32 and not SDL
  - **107** &middot; CMake + Ninja, built through CMake Presets
  - **113** &middot; Dependencies fetched by CMake, not vcpkg and not committed binaries
  - **122** &middot; Vulkan SDK installed properly, not headers-only
  - **126** &middot; Placeholder project name
  - **130** &middot; Terrain is single-valued, and that is a rule rather than a tuning
  - **140** &middot; Bare stone on ordinary ground is always a bug, and `else stone` is how you cause it
  - **150** &middot; Terrain and biome are outputs of the same fields; neither decides the other
  - **164** &middot; A user-adjustable frame cap, not vsync, and not uncapped
  - **177** &middot; Retired GPU buffers, not `vkDeviceWaitIdle`, once chunks stream
  - **187** &middot; A village grid is counted in blocks, and a villager's trade is claimed rather than assigned
  - **199** &middot; `VillageType` is a column on the biome table, not a question asked of tags
  - **205** &middot; Dear ImGui deferred from M11 to M34
  - **213** &middot; A fixed worker pool chosen at startup, not a resizable one
  - **223** &middot; Jobs own their inputs; they never point at the world
  - **229** &middot; A placeholder sun at M14c, and the real one at M24
  - **243** &middot; Which body is up is read off the light's own colour, not off a second flag
  - **251** &middot; The HUD had the world's pipeline, and then earned its own
  - **259** &middot; A distance tier is a rendering tier and must never become a simulation one
  - **273** &middot; A sub-allocator written here, not VMA
  - **281** &middot; Audio is stored as it arrives, not as the device wants it
  - **287** &middot; Block-entity contents are derived state on the screen, never owned by it
  - **295** &middot; Chests pair by derivation, not by stored state
  - **305** &middot; A stowbox carries its contents in the stack's `damage`, not in a new field
  - **315** &middot; Where water goes and what water is are two questions
  - **323** &middot; Waterlogging is a bit beside the block array, not a second block id
  - **329** &middot; Chunk saves and the player file carry separate version numbers
  - **335** &middot; An ore vein is a placed feature with a stated size, not a noise threshold
  - **343** &middot; A ported tuning value is only as good as the unit it was measured against
  - **351** &middot; A flier owns its own velocity, so nothing may push it
  - **357** &middot; Damage overwrites inside its window; it is not a cooldown
  - **365** &middot; `world/Survival.hpp` is header-only on purpose
  - **371** &middot; miniaudio, not SDL_mixer, OpenAL or FMOD
  - **384** &middot; A sound is named by material and by voice family, never by block or species
  - **392** &middot; No redstone signal engine, and it is a decision rather than a gap
  - **400** &middot; A potion is an item id per brew, not one item with a data value
  - **408** &middot; Brewing is instant, and the stand borrows the smithing screen
  - **414** &middot; Disc titles are ours, and a disc plays a note rather than a track
