...
**Index** - 45 sections. Line numbers, so a reader can jump straight there.
**Generated by `tools/index-docs.ps1`; do not hand-edit. Re-run it after editing this file.**

- **1** &middot; ANIMATION.md
  - **74** &middot; 0. Ground rules before any number is used
- **142** &middot; 1. The rig: pivots and hierarchy
  - **146** &middot; 1.1 Rotated, not translated — always
  - **150** &middot; 1.2 The pivot is at the joint, and which end that is varies by part
  - **168** &middot; 1.3 Hierarchy: it exists, and it is used sparingly
  - **200** &middot; 1.4 What rotating actually looks like versus translating
  - **222** &middot; 1.5 Exact pivot coordinates
- **304** &middot; 2. The walk cycle — the exact maths
  - **306** &middot; 2.1 `limbSwing` and `limbSwingAmount`, precisely
  - **336** &middot; 2.2 What `0.6662` means physically
  - **352** &middot; 2.3 The quadruped formula
  - **388** &middot; 2.4 The biped formula
  - **428** &middot; 2.5 Sprinting, sneaking, riding, babies
- **457** &middot; 3. Head movement
  - **459** &middot; 3.1 Two yaws, and the body chases the head
  - **493** &middot; 3.2 The clamps
  - **508** &middot; 3.3 Pitch
  - **534** &middot; 3.4 Head pivots
  - **540** &middot; 3.5 `look_at_player` and `random_look_around`
- **573** &middot; 4. Idle and secondary motion
  - **575** &middot; 4.1 What actually moves when a mob is standing still
  - **591** &middot; 4.2 The biped idle arm sway — the one universal idle
- **617** &middot; 5. Per-mob specifics
  - **619** &middot; 5.1 Chicken
  - **670** &middot; 5.2 Wolf
  - **728** &middot; 5.3 Spider ✅ **built, M20j (2026-08-06)**
  - **774** &middot; 5.4 Rabbit and frog — the hop
  - **795** &middot; 5.4b Swimmers — the squid's jet, and a tail that is not a leg
  - **856** &middot; 5.5 Bipeds — zombie arms and villager arms
  - **894** &middot; 5.6 Sheep and cow — grazing
- **945** &middot; 6. Bedrock's data-driven animation system
  - **947** &middot; 6.1 The three files
  - **997** &middot; 6.2 Blending — three separate mechanisms, and the distinction matters
  - **1005** &middot; 6.3 Honest engineering judgement: should we copy the shape at 36 species?
  - **1025** &middot; 6.4 One trend worth knowing: Java is moving toward keyframes too
- **1033** &middot; 7. Death, hurt and attack
  - **1035** &middot; 7.1 Death
  - **1062** &middot; 7.2 Hurt
  - **1073** &middot; 7.3 Melee attack swing ✅ **built, M20j (2026-08-06)**
  - **1122** &middot; 7.4 Ranged attack — the bow, and there is no wind-up
- **1179** &middot; What is worth taking, and what is not
  - **1181** &middot; (a) Should we add pivot-based rotation, and how big is it?
  - **1210** &middot; (b) The three highest gain-per-line improvements
  - **1246** &middot; Sources
...
## 7.4 Ranged attack — the bow, and there is no wind-up

Researched 2026-08-11 against `Mojang/bedrock-samples` `resource_pack/`. **This is the mob pose, not the player's**, and the two are different files with different numbers.

### The whole animation, verbatim
...
"animation.humanoid.bow_and_arrow": {
  "loop": true,
  "bones": {
    "leftarm":  { "rotation": [ "query.target_x_rotation - 90.0 - math.sin(query.life_time * 76.8) * 2.865 - this",
                                "query.target_y_rotation + 28.65",
                                "-(math.cos(query.life_time * 103.2) * 2.865) - 2.865" ] },
    "rightarm": { "rotation": [ "query.target_x_rotation - 90.0 + math.sin(query.life_time * 76.8) * 2.865 - this",
...
That is the **entire** entry. Two bones, no body, no head, no keyframes, no `animation_length`.

Rotations are **degrees**, `[x = pitch, y = yaw, z = roll]`, and **negative x raises an arm forward** — established inside the same file by `animation.humanoid.riding.arms` (−36 lifts the arms onto reins) and `animation.humanoid.sneaking` (+72 drops them back), so the sign is read rather than assumed.

### The five things worth knowing

1. **Both arms go to −90°, and they differ only in yaw.** Left **+28.65°**, right **−5.73°** — **34.38° apart**, which is what carries the string hand in across the chest. **There is no draw-back translation of the off hand.** The bow is on the **right**: this pose is the only one that also touches `rightitem`, `animation.bow.wield` binds there, and the attachable reads `query.main_hand_item_use_duration`.
2. **`- this` makes the pose an override, not an addition.** It replaces whatever the walk cycle put on that channel, so an aiming skeleton's arms stop swinging entirely. Adding a bow pose *on top of* a gait swing gives a walking archer paddling arms.
3. **⚠ The pose is gated on `query.has_target` and nothing else** — `controller.animation.humanoid.bow_and_arrow` is two states with transitions on `query.has_target` and `!query.has_target`, no charge term, no blend time, no length. **A skeleton does not wind up. It raises the bow the moment it acquires a target and holds it raised the whole time it has one**, and the 3.0 s reload (`RESEARCH.md` §17.6) happens behind a permanently-drawn bow. Anyone looking for a draw duration in the data will not find one, because there is not one.
4. **The idle jitter is the whole of the life in it**, and it is two channels: **±2.865° of pitch**, out of phase between the arms (`sin(life_time × 76.8)`, one `+` and one `−`), and **±2.865° of roll about a ±2.865° offset** (`cos(life_time × 103.2)`, mirrored). The rates 76.8 and 103.2 are deliberately incommensurate so it never visibly repeats — the same trick as the idle sway in §4.2, and the same trick our `swayRoll`/`swayPitch` already use at 1.8 and 1.34.
5. **Those constants are radians in disguise**, which is the strongest evidence the numbers are being read correctly: **2.865° = 0.05 rad**, 5.73° = 0.1 rad, 11.46° = 0.2 rad, 28.65° = 0.5 rad. The same family appears throughout §7.3.

### The pull stages are on the item, not on the arms

`controller.render.bow` selects one of **four** geometries and textures — `default` (standby) plus `bow_pulling_0/1/2` — by `query.get_animation_frame`. The attachable computes the charge itself:
...
**Divided by 10 ticks, against a bow whose full physics draw is 20.** That is primary-source confirmation of the split `RESEARCH.md` §17.2(2) states and that our `bowPullStage` encodes: **the visual pull finishes in half the time the physics charge does.** Never drive launch speed off the stage.

### What `swing: true` actually animates

The drowned's `minecraft:behavior.ranged_attack` carries `"swing": true` and the skeleton's does not. That flag drives `variable.attack_time`, which is exactly what `animation.humanoid.attack.rotations` in §7.3 consumes. So **the reference itself says a skeleton's shot moves no arm and a drowned's trident throw does** — an archer setting a swing timer is dead code by the reference's design, not only by ours.

`[JE]` ⚠ **A curiosity, recorded so nobody "fixes" it twice.** The shipped Molang for the mob's swinging arm reads `math.sin(1.0 - math.pow(1.0 - variable.attack_time, 3.0) * 180.0)` — which brackets as `sin(1 − (1−t)³ × 180)`, not the `sin((1 − (1−t)³) × 180)` that the shape obviously intends and that we implement. Ours matches the intent and the Java original; theirs is very probably a stray parenthesis in the data.

### What this means for us

**We have no locator system**, so a held bow is placed from the arm box's own far end — see `SYSTEM_MEMORY.md`. The reference's own `thirdperson_righthand` display transforms are the numbers to use for *which way an item sits in a fist*, and they are in `models/item/{handheld,bow,generated}.json` in the asset dump: `handheld` `[0, −90, 55]` at `[0, 4, 0.5]`/16 and scale 0.85, `bow` `[−80, 260, −40]` at `[−1, −2, 2.5]`/16 and scale 0.9, `generated` `[0, 0, 0]` at `[0, 3, 1]`/16 and scale 0.55. Rotations compose **X then Y then Z**, and the left hand is the same row with Y, Z and the sideways translation negated.
