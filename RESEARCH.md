...
**Index** - 113 sections. Line numbers, so a reader can jump straight there.
**Generated by `tools/index-docs.ps1`; do not hand-edit. Re-run it after editing this file.**

- **1** &middot; RESEARCH.md
  - **136** &middot; How to use this file
  - **159** &middot; Section status
- **283** &middot; 1. Entity physics
  - **290** &middot; 1.1 The tick model
  - **320** &middot; 1.2 Converting tick-space to delta time — the formulas we actually need
  - **347** &middot; 1.3 The per-entity constant table
  - **370** &middot; 1.4 Player movement — derivation of 4.317 m/s
  - **402** &middot; 1.5 Jumping
  - **425** &middot; 1.6 Step height and collision
...
**Read the status table below, then open only the rows it sends you to.** This file is three
thousand lines and no session has ever needed more than a handful of them. Every section carries
one of four labels, and the note beside it says what consumed the research or what is still owed:

| Label | What it means for you |
|---|---|
| **`INFO ALREADY USED`** | Implemented. Kept for reference — **do not re-read unless you are changing that system.** The note names the milestone, the file or the constant that consumed it. |
| **`ACTIVE`** | Not built, or only partly built. **These are the rows a future session should actually read.** |
| **`USED — BUT SEE audit.md`** | Implemented, but `audit.md` found the implementation wrong or incomplete. That is a *different* status from "not built": read the section **and** the named finding. |
| **`OUT OF SCOPE`** | Researched, then decided against. Kept rather than deleted, because "years from now" is not "never". |

Where a section is mostly one thing and partly another the label is the **dominant** status and the
note names the leftover, so `INFO ALREADY USED` never means "nothing left here" on its own. The same
labels are repeated **in place** as a banner under each top-level heading, and under the handful of
sub-sections whose status differs from their parent's — a banner and this table always agree.

> **Deleted 2026-08-17 — tombstone.** A hand-written table of contents sat here listing §1–§16 with
> anchor links. Removed as stale and duplicated: the generated index above already names every
> section with a line number, this table now carries the same list with a status, and the old one
> had never been updated to include §17, §18 or §19 at all.

## Section status

Every section of this file, in order, with what it is for and whether it has been spent.
**110 research sections: 80 `INFO ALREADY USED`, 17 `ACTIVE`, 11 `USED — BUT SEE audit.md`, 2 `OUT OF SCOPE`.**
(§7.5 carries an `OUT OF SCOPE` aside under a dominant `INFO ALREADY USED`, so grep finds three.)
Nothing here is wholly about the Nether, the End, redstone logic, ray tracing or global illumination
— all of which are out of scope — so those labels are rare; where such material appears it is a row
inside a table (§15.6, §15.7) or an aside, and it is called out in the note.

| § | What it is for | Status |
|---|---|---|
| **1** | **Entity physics — the tick model and every published constant** | **`INFO ALREADY USED`** |
| 1.1 | The 20 Hz fixed step, and the per-entity operation order | `INFO ALREADY USED` — the whole engine's fixed-step model |
| 1.2 | Tick-space → delta-time conversions; the `d^(Δt/0.05)` drag rule | `INFO ALREADY USED` — `world/Fluid.hpp`, `world/Projectile.cpp` |
| 1.3 | Gravity, drag and terminal speed per entity class | `INFO ALREADY USED` — arrows 0.05/0.99, items and falling blocks 0.04/0.98 |
| 1.4 | Where 4.317 m/s comes from | `INFO ALREADY USED` — player walk/sprint/sneak speeds |
| 1.5 | Jump strength 0.42 → apex 1.2522 m | `INFO ALREADY USED` — player and creature jumps derive from it |
| 1.6 | Step height, and Bedrock's push-out procedure | `INFO ALREADY USED` — per-species `stepHeight`, M20 |
| 1.7 | Entity-vs-entity collision is a soft horizontal impulse | `INFO ALREADY USED` — separation runs after world collision |
| 1.8 | Dropped-item physics, pickup box, despawn timer | `USED — BUT SEE audit.md` §11.2 — `ItemEntity.cpp:79` has no delta clamp, chunk guard or sweep, so drops fall through floors |
| 1.9 | Hitboxes, Bedrock first | `INFO ALREADY USED` — the species table's width/height columns |
| 1.10 | Fall damage, and the 23-vs-23.5 ordering artefact | `INFO ALREADY USED` — M21, `world/Survival.hpp` |
| 1.11 | Water and lava movement | `ACTIVE` — the water half shipped at M20h (`world/Fluid.hpp`); **lava movement is still unbuilt** — only `kLavaFogColour`/`kLavaFogDistance` exist |
| **2** | **Combat — damage, knockback, invulnerability, targeting, reach** | **`INFO ALREADY USED`** |
| 2.1 | Java's attack cooldown and charge meter | `OUT OF SCOPE` — Bedrock has no cooldown and Bedrock is the reference; kept only so nobody re-derives it |
| 2.2 | Weapon damage by tier | `INFO ALREADY USED` — `item/Item.hpp` |
| 2.3 | The knockback recipe (halve, apply, cap vertical at 0.4 b/t) | `INFO ALREADY USED` |
| 2.4 | The 0.5 s invulnerability window with the overwrite rule | `INFO ALREADY USED` — M21, and it is what makes melee survivable |
| 2.5 | `follow_range`, detection modifiers, line of sight | `INFO ALREADY USED` — M20f |
| 2.6 | Reach: 5 m blocks, 3 m entities | `INFO ALREADY USED` — taken verbatim 2026-08-05 |
| 2.7 | Difficulty scaling and regional difficulty | `OUT OF SCOPE` — the game runs on Normal only, and regional difficulty scales gear we do not have |
| **3** | **Health, hunger and food** | **`INFO ALREADY USED`** — M21 |
| 3.1 | Health, regeneration, the damage-source table | `INFO ALREADY USED` — `world/Survival.hpp` |
| 3.2 | Hunger as three values, and exhaustion as a tax | `INFO ALREADY USED` |
| 3.3 | The food table | `INFO ALREADY USED` — thirty-three foods carry these numbers |
| 3.4 | Armour points, toughness and the reduction formula | ✅ **USED 2026-08-19, fully as of 09:57** — values in `item/Item.hpp` (`armourDefence`, `armourToughness`), summed by `Inventory::armourSet`, spent by `Survival.hpp` `armourDamageTaken` inside `Player.cpp` `damagePlayer`, reached from combat because `Main.cpp`'s `hurtPlayer` takes a **non-defaulted** `ArmourSet`, worn through `wearArmour`, saved at player format v7. *Was `ACTIVE`: "nothing wears armour and the formula is unimplemented"; then, at 06:44, "one gap left — `hurtPlayer` omits the `ArmourSet` argument".* **Still unspent from the sheet below: the durability column** — no armour piece has a row in `mining::toolProperties`, so a set never breaks |
| 3.5 | Absorption: 4 points a level, spent first, never refilled, gone with the effect | `INFO ALREADY USED` — 2026-08-18; `Player::absorption` is the pool and the HUD draws it. The **totem grant** is the leftover, there being no totem |
| **4** | **Mob spawning and despawning** | **`USED — BUT SEE audit.md`** |
| 4.1 | Bedrock's spawn shell, caps and cluster spawning | `USED — BUT SEE audit.md` §13.4 — the continuous spawner falls back to **Sheep** when the weighted roll misses; per-category caps are also still one shared number |
| 4.2 | Spawning at chunk generation | `INFO ALREADY USED` — built as a pure function of `(seed, chunkCoord)`; the heading's "biggest gap for us" is stale |
| 4.3 | Despawning and persistence | `INFO ALREADY USED` — `creatures.dat`; ours retires abruptly at 90 m rather than thinning at 1/800 a tick |
| **5** | **Mob drops, breeding and babies** | **`INFO ALREADY USED`** |
| 5.1 | Drop ranges, rare drops, looting, the 5 s XP rule | `INFO ALREADY USED` — minus XP, which does not exist |
| 5.2 | The drop table for our roster | `INFO ALREADY USED` |
| 5.3 | Breeding, growth and baby rates | `ACTIVE` — `babyChance` at spawn is built; **feeding two adults to breed is not** — there is no love/cooldown state anywhere in `game/src` |
| **6** | **Mob behaviour — the species we have** | **`INFO ALREADY USED`** |
| 6.1 | HP, speed and temperament per species | `INFO ALREADY USED` — the `Creature.cpp` species table |
| 6.2 | Per-species quirks worth implementing | `INFO ALREADY USED` — most are in; chicken egg-laying, the goat's ram and the camel's dash are named as still missing inside the section |
| 6.3 | The creeper, our Bramble's source, and the blast formula | `USED — BUT SEE audit.md` §13.7 — blast damage to the player sits inside the knockback-direction guard. ⚠ Its swell distances are **2.5 / 6**, not the 3 / 7 in the body text; see the note at the section's end before touching them |
| **7** | **Mob behaviour — what we could add (the four archetypes)** | **`INFO ALREADY USED`** |
| 7.1 | Archetype A, land quadrupeds | `ACTIVE` — the verdict was spent on fox, ocelot, polar bear and panda; **turtle, armadillo, sniffer and hoglin rows are unspent** |
| 7.2 | Archetype B, flying | `INFO ALREADY USED` — M20b, the bee; `flies` is the mirror of `swims` |
| 7.3 | Archetype C, swimming | `INFO ALREADY USED` — M20i, nine species plus the drowned |
| 7.4 | Archetype D, bipedal — and what a villager really costs | `INFO ALREADY USED` — M29d bought the whole list: village generator, claimable blocks, schedule, trades, iron golem |
| 7.5 | Naming — which names are free and which are coined | `INFO ALREADY USED` — Bramble, Blackbone, Princepin, Emberite, stowbox. The wider asset/legal worry it was written under is `OUT OF SCOPE` and **retired**: the repo is private, the game is personal-use, and the reference textures stay |
| **8** | **The Bedrock AI architecture** | **`INFO ALREADY USED`** — M20c |
| 8.1 | Priority + `control_flags`, and why that is the whole trick | `INFO ALREADY USED` — `Move`/`Look`/`Jump` arbitration |
| 8.2 | A real vanilla goal ordering, the chicken | `INFO ALREADY USED` — the shape our behaviour table copies |
| 8.3 | Component parameters worth knowing | `INFO ALREADY USED` — scan interval, `alert_same_type`, `melee_fov` are all in |
| 8.4 | Navigation vs movement, and the seven navigation flags | `INFO ALREADY USED` — `swims`/`flies`/`amphibious`/`avoidsWater` are this split |
| 8.5 | Pathfinding node costs and the −1 sentinel | `INFO ALREADY USED` — M20k, `world/Pathfinder.hpp` |
| 8.6 | Spawn rules as a declarative condition list | `ACTIVE` — this is §8.8's step 5, and it is the one step still open: the two spawners are not unified |
| 8.7 | What to take and what to skip | `INFO ALREADY USED` — all four "take" items are in; all five "skip" items stayed skipped |
| 8.8 | The migration path, step by step | `INFO ALREADY USED` — steps 1–4 built; **step 5 open, step 6 deliberately deferred** |
| **9** | **Blocks, fluids and plants** | **`INFO ALREADY USED`** |
| 9.1 | Fluid mechanics | `INFO ALREADY USED` — already reduced to a pointer at M20h; the four unrecoverable findings are the only content left |
| 9.2 | Gravity blocks and the landing rule | `INFO ALREADY USED` — `world/FallingBlock.cpp`, 2026-08-07 |
| 9.3 | Crop growth, the exact formula | `USED — BUT SEE audit.md` §12.1 — `World.cpp:683` passes chunk-local Y as world Y, so **crops never grow at or above y=32** |
| 9.4 | Random ticks, the clock everything runs on | `INFO ALREADY USED` — growth, fire and farmland all ride it |
| 9.5 | Leaf decay | `ACTIVE` — **not built.** No distance/persistent state exists; player-cut trees keep their canopies |
| 9.6 | Grass spread | `ACTIVE` — **not built.** Dirt exposed by mining never greens over |
| 9.7 | Solid vs opaque vs full block — six independent properties | `INFO ALREADY USED` — `BlockShape`, `occludesFace`, `restsOnWater`, `canWaterlog` are this table |
| 9.8 | Stairs and slabs | `INFO ALREADY USED` — M20w, `ShapedFamily`; corner shapes are not derived and that is deliberate |
| 9.9 | Fire | `INFO ALREADY USED` — with a **named divergence**: `World.cpp` uses a flat `kFireSpreadPercent = 25` roll, not the degree formula |
| **10** | **Mining, tools, smelting, XP** | **`USED — BUT SEE audit.md`** |
| 10.1 | The mining formula, hardness table, ×1.5 / ×5 | `USED` — the run-5/6 hardness gap audit.md §13.5 recorded is **fixed**: the table moved to `Mining.hpp` `miningRow`, whose `default:` returns the `kUnchosenHardness` sentinel rather than a plausible 1.0, and a sweep invariant fails the build on any row still carrying one (re-checked 2026-08-19) |
| 10.2 | Smelting times, fuel values, XP on collection | `INFO ALREADY USED` — the furnace, blast furnace and smoker |
| 10.3 | Ore drops and XP per ore | `INFO ALREADY USED` — ⛔ its closing note is **stale**: we have five tool tiers now, not "only wood and stone", and each ore demands the reference's own tier |
| 10.4 | Experience | `ACTIVE` — **XP does not exist.** `item/Recipe.hpp`'s `repairResult` doc comment (line 116 as of 2026-08-19; the `:123` this replaced was past the end of a 120-line file) says so in as many words |
| **11** | **Crafting and recipes** | **`INFO ALREADY USED`** |
| 11.1 | Shaped/shapeless matching, and ingredient tags | `INFO ALREADY USED` — ⛔ its prediction is **contradicted by the code**: tags were never built, and 1,152 recipes are generated in `item/Recipe.cpp` loops instead |
| 11.2 | The core recipe shapes, stonecutter, smithing, stack sizes | `INFO ALREADY USED` — all three benches are built |
| **12** | **Light — four quantities, two channels** | `INFO ALREADY USED` — both channels ride the vertex colour; **internal sky light is the one idea still unspent**, so a thunderstorm cannot let hostiles spawn by day |
| **13** | **Time, weather and sky** | **`INFO ALREADY USED`** |
| 13.1 | The day, its phases, and the hostile spawn windows | `INFO ALREADY USED` |
| 13.2 | The non-linear sky-angle formula | `ACTIVE` — **not taken.** `world/Sky.cpp:62` is `timeOfDay * 2π`, a plain linear sweep; this is the one line that makes sunsets linger |
| 13.3 | Weather as two flags and two timers | `INFO ALREADY USED` — `world/Weather.hpp` carries these eight timer bounds verbatim |
| 13.4 | Burning in sunlight, and who is exempt | `INFO ALREADY USED` — `burnsInDay` follows this list species by species |
| **14** | **Ticking, chunks and persistence** | **`ACTIVE`** — the least-spent chapter in the file |
| 14.1 | The reference's per-tick order | `ACTIVE` — ours differs; the ordering facts are still an unspent argument |
| 14.2 | Scheduled versus random ticks | `ACTIVE` — random ticks are built; **scheduled ticks saved with the chunk are not** |
| 14.3 | Block updates are depth-first, not queued | `ACTIVE` — the Post-Placement order matters for connected shapes; the Neighbor-Changed order exists for redstone, which is out of scope |
| 14.4 | Chunk load levels as one propagating integer | `ACTIVE` — we have a binary loaded/not-loaded plus `World::columnResident`; a level would encode "generated but not simulated" directly |
| 14.5 | Region files, sectors and compression | `ACTIVE` — we have our own format, and `audit.md` B-New-1 says our chunk files are **completely uncompressed**; the compression-type byte here is the model |
| **15** | **World generation — the whole 1.18 architecture** | **`INFO ALREADY USED`** — M20p–M20r |
| 15.1 | The five stages, and why a biome no longer decides height | `INFO ALREADY USED` — the rebuild's founding idea |
| 15.2 | Climate parameters and how a biome is chosen | `INFO ALREADY USED` — five fields, 27 biomes claiming boxes |
| 15.3 | Seed, noise and sampling resolution | `INFO ALREADY USED` — per-noise seeding |
| 15.4 | Terrain shaping: `offset`, `factor`, `jaggedness` | `INFO ALREADY USED` — and `factor` is the constant that stops spires; see §15.10 before changing it |
| 15.5 | Surface rules | `INFO ALREADY USED` — a top-down walk over the uncarved column |
| 15.6 | Caves and aquifers | `INFO ALREADY USED` — three cave systems; **aquifers deliberately not taken** |
| 15.7 | Ores, features and the placement RNG | `INFO ALREADY USED` — ore veins as placed features; the feature-index seed scheme is Java's and was not adopted |
| 15.8 | What else a biome carries — spawners, colour, temperature | `INFO ALREADY USED` — including the standing "do not biome-tint our foliage" fact |
| 15.9 | ▶ Where we stand | `INFO ALREADY USED` — already carries its own ⛔ banner: superseded the day it was written, kept because the comparison is the argument for the rebuild |
| 15.10 | The four rules that keep a generated world sane | `INFO ALREADY USED` — every one came from a playtest bug; **read this before touching terrain**, it is the only part of §15 that is still load-bearing |
| **16** | **Priority list — what to take first** | `INFO ALREADY USED` — **every row is now closed**; kept as a record of what the order turned out to be |
| **17** | **Projectiles, the bow and the arrow** | **`USED — BUT SEE audit.md`** |
| 17.1 | Every arrow number, and its unit | `USED — BUT SEE audit.md` §13.1 — the numbers are right in `Projectile.cpp`, but there is no player in it at all, so **a skeleton's arrow can never hit you** |
| 17.2 | The five non-obvious things (quadratic draw, speed-derived damage) | `INFO ALREADY USED` — verified against the closed form to one part in a million |
| 17.3 | Swept collision, blocks before entities | `USED — BUT SEE audit.md` §13.2 — a mob's arrow that hits another creature is blamed on **you** |
| 17.4 | The arrow model — three cubes, two of them flat | `INFO ALREADY USED` |
| 17.5 | What we deliberately do not have | `INFO ALREADY USED` — a divergence register, not research |
| 17.6 | `behavior.ranged_attack`, and the seconds-not-ticks warning | `INFO ALREADY USED` — M29l corrected our interval from 1 s to 3 s on the strength of it |
| 17.7 | The witch's potion ladder and its drink goal | `INFO ALREADY USED` — M29l, splash potions thrown rather than a walk-in attack |
| 17.8 | The drowned's trident, gated three ways | `ACTIVE` — **not built.** The `Trident` item exists; nothing throws one |
| **18** | **Thrown items: the pearl and the egg** | **`INFO ALREADY USED`** — M20y, the void pearl |
| 18.1 | The three-file component table | `INFO ALREADY USED` — the pearl's inertia of exactly 1 is the load-bearing fact |
| 18.2 | What is genuinely undocumented | `INFO ALREADY USED` — no in-air lifetime, no placement rule |
| 18.3 | Fall damage, cooldown, stack size, and our three divergences | `INFO ALREADY USED` |
| **19** | **Villages, villagers and the iron golem** | `USED — BUT SEE audit.md` §13.3 — M29d built all of it, but **a villager's bed/job/bell claim is never saved**, so villages stop working after a reload; see also §13.10 and §13.11 |
| — | **Sources** | `ACTIVE` — bibliography, not research. Open it only to re-check where a number came from |

---

# 1. Entity physics

> ✅ **INFO ALREADY USED.** Spent across M20, M20h and M21 — the fixed step, the drag conversion,
> the 4.317 m/s derivation, jump height 1.2522, per-species step height, soft entity separation and
> fall damage are all in the game. Kept for reference. **The exception is §1.11's lava half**, which
> is still unbuilt.
...
> ⚠ **USED — BUT SEE `audit.md` §11.2.** The physics and the five-minute despawn are in, but
> `ItemEntity.cpp:79` has no delta clamp, no unloaded-chunk guard and no swept collision, so a
> dropped item can fall through the floor.
...
> **ACTIVE — half spent.** Every water number shipped at M20h and now lives in `world/Fluid.hpp`.
> **Lava movement did not**: the only lava constants in the codebase are `kLavaFogColour` and
> `kLavaFogDistance`, so the drag ≈ 0.5, the 0.784 m/s walk and the lava current are all still owed.
...
>
> ⛔ **Correction, 2026-08-17.** All three "things to take" were taken. Drag is in `world/Fluid.hpp`
> and `world/Projectile.cpp`, the separation impulse runs after world collision, and the despawn
> timer is `ItemEntity.cpp:52` — `kDespawnSeconds = 300.0f`, the reference's five minutes. Those
> three files are authoritative; the paragraph above is a snapshot of 2026-08-08.

---

# 2. Combat

> ✅ **INFO ALREADY USED.** Damage tiers, knockback, the 0.5 s invulnerability window with the
> overwrite rule, `follow_range` with the sneak modifier, and 5 m/3 m reach are all in — M20f, M20j
> and M21 between them. Kept for reference. **§2.1 and §2.7 are `OUT OF SCOPE`**, marked in place.

## 2.1 Attack cooldown — Java only

> ⛔ **OUT OF SCOPE.** Bedrock has no attack cooldown and Bedrock is the reference, so this whole
> system is skipped by decision. Kept so nobody re-derives it from the Java wiki and "fixes" combat.
...
> ⛔ **OUT OF SCOPE.** The game runs on Normal and has no difficulty setting; regional difficulty
> scales gear and enchantments that do not exist. Kept in case a difficulty option is ever wanted.
...
> ✅ **INFO ALREADY USED.** Built as M21; `world/Survival.hpp` owns every number in §3.1–§3.3 and
> the food table is thirty-three rows in the item table. Kept for reference. **§3.4 armour was
> spent on 2026-08-19** — slots, `armourSet`, the reduction formula and durability wear are all
> live; only `Main.cpp`'s `hurtPlayer` still passes no set.
...
**The design insight worth internalising: hunger is not a food timer, it is a tax on healing and hurrying.** Walking is free. Sprinting 100 blocks costs 10 exhaustion = 2.5 food. Healing twenty points — ten hearts, half a heart at a time — costs 120 exhaustion = **30 food**, which is one and a half full bars, by far the most expensive thing a player can do.

> **Corrected 2026-08-18.** This paragraph read *"Sprinting 100 blocks costs 30 exhaustion = 7.5
> food. Healing ten hearts costs 60 exhaustion = 15 food"* and **contradicted its own table two
> lines above it**, which says 0.1 per metre and 6.0 per HP. The table is right: the wiki's
> exhaustion table gives sprinting **0.1 per metre** and regeneration **6.0 per HP healed**,
> and 4.0 exhaustion is one leg of the bar
> ([https://minecraft.wiki/w/Food#Exhaustion]). So 100 m is 10 exhaustion, and ten hearts is
> twenty *points* at 6.0 each, not ten. `Player.cpp` and `survival::kExhaustSprintPerMetre` were
> already right — **the prose was the error, and making the code match it would have been the
> expensive way round.** Worth keeping as a tombstone because a worked example is exactly what a
> reader trusts over a table.
...
> ✅ **USED 2026-08-19 — values taken *and* mechanic taken, and as of 09:57 the last wire is in.**
> `item/Item.hpp` carries `armourDefence` and `armourToughness` for all twenty-four pieces, straight
> off the table below; `Inventory` has four equipment slots and `armourSet()` sums them (rejecting a
> piece worn in the wrong slot); `Survival.hpp` `armourDamageTaken` implements the formula below and
> `Player.cpp` `damagePlayer` applies it, charging `armourDurabilityCost` on the raw incoming damage;
> `Main.cpp` drains `Player::armourWear` into `Inventory::wearArmour`; and the worn set is saved as
> `SavedPlayer::armour` at **player format version 7**.
> *Was: "there are no armour slots and no damage reduction; nothing in `game/src` calls either
> function." That was true until roughly 06:00 on 2026-08-19 and is now false.*
>
> **The fourth argument is spent too, and the way it was spent is the part worth copying.** *Was, at
> 06:44: "the one thing still unspent is the fourth argument — `damagePlayer`'s `ArmourSet` defaults
> to `kNoArmour` and `Main.cpp`'s `hurtPlayer` passes only two arguments, so creature blows, arrows,
> explosions, lightning and instant-harming potions are all unarmoured."* `hurtPlayer` now **takes an
> `ArmourSet` with no default**, which makes the compiler name every damage site rather than letting
> the next one inherit the silence; five of its six sites pass `inventory.armourSet()` and the sixth
> passes `kNoArmour` at an explicit call site because Bedrock routes a Harming potion through `magic`.
>
> **Three things are still missing, and none is a research question:** no piece has a durability row
> in `mining::toolProperties`, so a set wears and never breaks; there is no equip sound event of any
> kind (the reference plays `item.armor.equip_leather`/`_iron`/`_diamond`/`_netherite`/`_generic`);
> and the HUD has no armour bar. Fall damage (`chargeFall`) is unarmoured and **that one is still
> unsourced — nobody has checked Bedrock. Do not decide it from memory.**
...
## 3.5 Absorption — the only padding in the game

> ✅ **INFO ALREADY USED.** Landed 2026-08-18. `effects::absorptionPoints` owns what a grant is
> worth, `Player::absorption` is the live pool, `damagePlayer` spends it before health,
> `tickPassiveTimers` clears it with the effect, and `hud::makeStatusBars` draws it as gold hearts
> above the health row. **Leftover: the totem grant below**, because there is no totem of undying
> in the game, and negative levels, which nothing can produce.

Absorption adds **4 health points per level** — two hearts — and they sit **on top of** the twenty, not inside them. Everything else about them follows from their being a **balance rather than a maximum**:

- **They are depleted first**, before the standard health points, and after every reduction that decides how big the blow is. Sixteen points of padding buys sixteen *resisted* points.
- **They cannot be replenished** — not by natural regeneration, not by a healing potion, not by any other effect. Only a fresh grant restores them, and a grant may only ever raise the balance.
- **They vanish when the effect ends.** The pool is not carried into the next one, so two golden apples two minutes apart do not stack.
- Displayed as **yellow hearts above the normal health bar**.

| Source | Level | Duration |
|---|---|---|
| Golden apple | I | 2:00 |
| Enchanted golden apple | IV | 2:00 |
| Totem of undying | II | 0:05 |

Everything in this section is identical in both editions, so there is nothing to convert. [https://minecraft.wiki/w/Absorption]

**The trap this section exists to name: "what a grant is worth" and "what is left" are different numbers, and only one of them is a function of the running effects.** The effect is still at full level when the pool is empty — that is what "cannot be replenished" *means* — so anything that reads the level to answer "how much padding does the player have" reports a full eight hearts to a player who has none. The HUD row read the balance from the first line it existed; the function that returns the level's worth is called in exactly one place, when a grant arrives.
...
>
> ⛔ **Correction, 2026-08-17.** "Not taken: … potions" is stale. **Brewing and status effects
> shipped at M29i** — `world/Effects.hpp` owns them and `item/Item.hpp` carries 41 potion types in
> three runs (drinkable, splash, tipped arrow). Armour, difficulty levels and experience are still
> genuinely not taken.
...
> ⚠ **USED — BUT SEE `audit.md` §13.4.** Both of the reference's spawn systems run, and
> `creatures.dat` persists the population. Two things are wrong or missing: the **continuous
> spawner falls back to `Sheep`** when its weighted roll misses (the chunk spawner does not), and
> we still have **one cap for every category** where Bedrock splits Monster / Animal / Ambient.
...
> ✅ **INFO ALREADY USED**, with one gap. Drop ranges, the per-species drop table and `babyChance`
> in a naturally spawned group are all in. **§5.3's breeding loop is `ACTIVE` and unbuilt** — there
> is no love state, no feeding, no parent cooldown and no growth timer anywhere in `game/src`.
...
| Rabbit | 1–3 | **0–1 raw rabbit** `[JE 1]`; 0–1 rabbit hide; rabbit's foot at 10% |
| Cat | 1–3 | 0–2 string |
...
> **ACTIVE — not built.** `babyChance` at spawn is in (`world/Creature.cpp`), so babies exist and
> grow, but **nothing below this line is implemented**: no love state, no breed item, no
> pathfind-to-partner, no five-minute cooldown, no baby from two parents. Grep found no love or
> cooldown field anywhere in `game/src`. This is the whole of the mechanic, ready to take.
...
> ✅ **INFO ALREADY USED.** The master table is the `Creature.cpp` species table, and the
> per-species quirks were spent across M20b–M20j. Kept for reference. **§6.3 additionally carries
> `USED — BUT SEE audit.md`** and a standing do-not-flip warning about its swell distances.
...
> ⚠ **USED — BUT SEE `audit.md` §13.7.** The Bramble and `world/Explosion.hpp` were built from this
> section on 2026-08-04; the audit found **blast damage to the player sitting inside the
> knockback-direction guard** while damage to creatures does not.
>
> ⚠ **The two swell distances below are the wiki's, and the wiki is wrong here.** Bedrock's own
> `creeper.json` says **2.5 and 6**, not the 3 and 7 in points 2 and 5. Read the note at the end of
> this section before touching them — they have already been flipped the wrong way once.
...
> ✅ **INFO ALREADY USED.** All four archetypes were bought — quadrupeds at M20b, flying at M20b,
> swimming at M20i, bipedal at M29d with the villager and the iron golem — and the naming split in
> §7.5 is what Bramble, Blackbone, Princepin, Emberite and stowbox came out of. **§7.1's species
> rows are the one unspent part**: turtle, armadillo, sniffer and hoglin are still unbuilt.

## 7.1 Archetype A — land quadrupeds *(reuses everything we have)*

> **Partly `ACTIVE`. Built since this was written:** fox, ocelot, polar bear and panda are in the game, and the verdict held — zero new movement code, zero new physics. What each cost was a measured net and a placement pass. Turtle, armadillo, panda variants, sniffer and hoglin remain.
...
> ✅ **INFO ALREADY USED**, and its wider worry is **retired**. The coined-versus-ordinary split is
> shipped — sheep is a sheep, and the Creeper is a Bramble. The asset/legal anxiety this was written
> under no longer applies: the repo is private, the game is personal-use, and the reference textures
> and sounds stay permanently. Do not reopen that question.
...
> ✅ **INFO ALREADY USED.** §8.8's steps 1–4 were built at M20c and the pathfinder at M20k — the
> switch is gone, behaviours sit in a `constexpr` table with `Move`/`Look`/`Jump` control flags, and
> targeting is split from attacking. Kept for reference. **Step 5 — unifying the two spawners
> behind §8.6's declarative spawn rules — is the one step still open**, and step 6 is deferred.
...
> **ACTIVE.** This is §8.8's step 5 and it is the last step still open: the chunk-generation spawner
> reads a species table while the continuous cycle reads its filters inline. **Read this section
...
> ✅ **INFO ALREADY USED**, with three exceptions marked in place. Fluids (M20h), gravity blocks,
> random ticks, the six shape properties, stairs and slabs (M20w) and fire are all built. **§9.5
> leaf decay and §9.6 grass spread are `ACTIVE` and unbuilt**, and **§9.3 crop growth is
> `USED — BUT SEE audit.md`.**
...
> ⚠ **USED — BUT SEE `audit.md` §12.1.** `world/Farming.hpp` carries this formula exactly, and the
> three-by-three farmland patch under the plant is where the points come from. But `World.cpp:683`
> passes a **chunk-local Y as a world Y**, so **nothing grows at or above y=32**.
...
> **ACTIVE — not built.** No leaf carries a distance or a persistent flag, so a felled trunk leaves
> its canopy hanging for ever. This section is the whole spec if that is ever wanted.
...
> **ACTIVE — not built.** Dirt exposed by mining stays dirt for ever. Note the asymmetry below —
> three down, one up — which is what makes grass creep downhill faster than up.
...
> **USED — and the `audit.md` §13.5 gap is now closed.** The mining formula, the hardness table,
> the ×1.5/×5 split, smelting times, fuel values and ore drops are all in. The run-5/6
> `harvestTier` gap the audit recorded has been **fixed**: the tables moved out of `Tool.cpp`
> (now three one-line forwarders) into `Mining.hpp` `miningRow`, whose `default:` returns the
> `kUnchosenHardness` sentinel rather than a plausible 1.0, and a sweep invariant fails the build
> on any row still carrying one. Beacon, conduit, monster spawner, dragon egg, end portal frame
> and blast furnace all have explicit rows; the trapped chest is covered by `isChest`.
...
> ⛔ **Correction, 2026-08-17.** The sentence above is stale. There are **five tool tiers** now —
> wood, stone, iron, diamond and Emberite — and **each ore demands the reference's own tier**; the
> one-step-lower compromise was retired when the tiers landed. Nine ores generate, not eight.
> **⚠️ Second correction, 2026-08-19: `item/Tool.cpp` is no longer authoritative and must not be
> read as such.** It is 77 lines holding three forwarders (`toolFor`, `blockHardness`, `yieldsDrop`)
> and no table of any kind. **`item/Mining.hpp` `kMiningRows` owns hardness, tool kind, tier and the
> drop gate**, one `MiningRow` per block id, sized by `kBlockIdCount`. The five tier constants
> `kHandTier`…`kEmberiteTier` do still live in `item/Tool.hpp`. Only "XP does not exist yet" is
> still true.
## 10.4 Experience

> **ACTIVE — not built.** There is no XP in the game: no orbs, no levels, no furnace bank.
> `item/Recipe.hpp`'s `repairResult` doc comment (line 116 as of 2026-08-19; the `:123` this replaced was past the end of a 120-line file) says so where it explains why repair costs nothing.
...
> ✅ **INFO ALREADY USED.** The 3×3 grid, shaped and shapeless matching, the stonecutter, the
> smithing table and the stack-size rules are all built, and 1,152 recipes stand against them.
> ⛔ **One prediction here is now contradicted by the code** — see §11.1.

## 11.1 Matching rules

> ⛔ **Correction, 2026-08-17.** The note below says ingredient tags "we will want the moment a
> second wood type exists — and adding them retroactively means touching every recipe." **Eleven
> woods exist and tags were never built.** `item/Recipe.cpp` generates the families in loops
> instead, which solved the same problem from the other end. `game/src/item/Recipe.hpp` is
> authoritative for how matching actually works here.
...
**Stonecutter:** one input, pick an output. Strictly better than crafting for stone stairs (1→1 vs 6→4).

> **▶ Where we stand (2026-08-10).** **Built.** One input and up to three previewed cuts — stairs, two slabs, a wall — at exactly those rates. **Which cuts a block has is a reverse lookup over the family tables**, not a list of its own, so a new stair material offers its cuts with no second table to keep in step. The reference's chiselled and polished variants are not offered, because they are separate blocks rather than cut shapes.

**Smithing table:** template + item + material, preserving durability and enchantments at no XP cost.
...
> ✅ **INFO ALREADY USED.** Both channels ride the vertex colour's red and green with face shade ×
> ambient occlusion in blue, and the sky-light free-fall rule is in with `isSkyTransparent` split
> from `isLightTransparent`. **The one unspent idea is internal sky light** — ours is a `night`
> boolean and a `skyLightAt` read, so a thunderstorm can never let hostiles spawn by day.
...
> ✅ **INFO ALREADY USED.** The day and its phases, the two-flag weather machine (`world/Weather.hpp`
> carries these eight timer bounds verbatim) and `burnsInDay` are all built. **§13.2's non-linear
> sky-angle formula is `ACTIVE` and was never taken** — marked in place.
...
> **ACTIVE — not taken.** `world/Sky.cpp` sweeps the sun linearly: `angle = timeOfDay * 2π`. The
> formula below is why the reference's sunsets linger and its noons pass quickly, and it is one
> line. The square sun and moon quads, the always-opposite moon and the 7° horizon fade are equally
> unspent.
...
> **ACTIVE — the least-spent chapter in this file.** We have multithreaded generation and meshing,
> chunk streaming, random ticks and a save format, but **none of the four big ideas here were
> taken**: the load level as one propagating integer, scheduled ticks saved with the chunk, the
> reference's tick ordering, and the region file's compression byte — which `audit.md` B-New-1 says
> we need, since our chunk files are stored completely uncompressed.
...
> ✅ **INFO ALREADY USED — and it is the largest single purchase in this file.** The generator was
> rebuilt on this architecture at **M20p** and corrected twice by playtesting at **M20q** and
> **M20r**: five climate fields, `offset`/`factor`/`jaggedness` splines, height computed directly
> per column, thirty biomes claiming boxes, three cave systems, ores as placed veins.
> `SYSTEM_MEMORY.md` → "World Generation" is the current truth. **Aquifers, the badlands clay table,
> `preliminary_surface_level` and the 7D R-tree were all deliberately not taken.**
>
> **If you read one part of §15, read §15.10** — its four rules are guarantees rather than tunings,
> every one came from a playtest bug, and undoing one puts floating land or rings of bare stone back
> in the world.
...
> air-exposure discard on the deepest three; and **thirty** biomes each
...
>
> **How to write a claim in this file, and it is not optional — a dated claim with a named falsifier is a different kind of object from a claim.** Established in the field on 2026-08-19 in under half an hour: "honeycomb is unobtainable" was measured correctly at 09:59 by a graded control, written into five documents, and became **false at 10:23:32** when `Main.cpp` gained the consumer. It was caught **only** because the claim had shipped with its own falsifier attached — *"this becomes false the moment a consumer of `takePollinated` appears in `Main.cpp`"* — so re-checking it was one grep against a named file rather than a re-audit. **The bare claim would still be sitting in five documents right now.** So: **any sentence saying something does not exist, is unobtainable, is unreachable, or has no callers must carry (a) the date it was measured and (b) the single observation that would refute it.** Negative claims rot fastest, they rot silently, and they are the ones that send an agent to build something twice. A claim you cannot name a falsifier for is one you have not finished checking.
>
> **⚠ `fbm2D` and `fbm3D` no longer exist and must not come back.** They were deleted on 2026-08-19 because they were octaved value noise returning **[0, 1]** sitting one letter away from `octaves2D`/`octaves3D`, which return **[−1, 1]** — measured, a `> 0` test passes 49.8% of the time on the live pair and **100%** on the deleted one. Everything below is written against the live pair. The three paragraphs that follow were rewritten on 2026-08-19 from the source, because the versions they replaced described a generator that had not existed since 2026-08-07 and named `fbm2D` as its engine — a reader could have re-added one *to satisfy this document* and re-armed exactly that trap. **The old text quoted frequencies (0.0022, 0.010, 0.018); the code is written in wavelengths (84, 210, 58, 104, 27 blocks). Those are reciprocals**, so no number may be carried between the two without inverting it — and none was, because none of the current values is the reciprocal of an old one.
>
> **Biomes.** **Thirty**, in [game/src/world/Biome.cpp](game/src/world/Biome.cpp) — `kBiomes` is sized off `BiomeId::Count` and a `static_assert` proves every enumerator has a row, so this count cannot drift from the code. Each row claims a **box in five dimensions** — temperature, humidity, continentalness, erosion and weirdness — resolved **first match wins, then nearest** when a point falls in no box at all. `ridges` is the sixth axis a row can constrain and is *derived* rather than sampled: `1 − |3|w| − 2|` folds the weirdness field so one smooth noise gives two valley lines and one peak line per period, which is what makes mountain ranges read as ranges. Every field comes from `climateAt` in [game/src/world/Climate.cpp](game/src/world/Climate.cpp) as an `octaves2D` sum in [−1, 1], each with its own **amplitude array** rather than a plain octave count — a zero entry drops that octave outright, so continentalness is `{1,1,2,2,2,1,1,1,1}` and temperature is `{1.5,0,1,0,0,0}`, and neither is expressible as "n octaves". A `BiomeTag` bitmask lets creature and feature rules ask for a *property* instead of naming biomes. This is the reference's own 1.18 structure at full dimensionality, with a hard winner rather than soft weights.
>
> **Height** is **not** a global noise field that biomes scale, and a biome no longer knows how tall it is — that inversion is the whole point of the 1.18-style rewrite. `shapeAt` in [game/src/world/Climate.cpp](game/src/world/Climate.cpp) reads **continentalness, erosion and ridges only — never temperature and never humidity** — and evaluates four splines into three numbers: `offsetY` is `kContinentalOffset(continentalness)` plus `kErosionRelief(erosion) × kPeakProfile(ridges)` scaled by a coastal `inland` ramp, minus a river cut taken from the ridge field's own zero contour (so rivers need no river system); `factor` is `kErosionFactor(erosion)`, blended to `kOceanFactor` across the same coastal ramp; `jaggedness` is a **0-to-1 weight**, non-zero only where ridges are high *and* erosion is low, which both adds `kJaggedBlocks` of high-frequency height and pulls `factor` toward `kRuggedFactor`. **`factor` is not a height, it is how hard `offsetY` is enforced:** the surface lands at `offsetY + octaves3D × kReliefBlocks / factor`, with `kReliefBlocks = 22` and the 3D field running at `kBaseWavelengthXZ = 84` blocks horizontally and `kBaseWavelengthY = 210` blocks vertically. That single split is what lets flat plains and jagged hills exist at the same temperature, humidity and altitude. **Surface blocks come from the winning biome**, because a blend of two block types is not a thing; altitude and the waterline override the biome's own top block afterwards.
>
> **World geometry.** `kWorldHeightChunks = 3` × `Chunk::kSize = 32` → **y 0–95** (`kWorldHeight = 96`, `kWorldTop = 95`), sea level **24**, surface clamped to **`kMinSurface = 4` … `kMaxSurface = 90`** — though `kMinSurface` is a **dead clamp**, the measured minimum over 1,048,576 columns being y 7 (finding 981). Against the reference's −64…319 with sea level 63, depths below sea level compress by about **0.17** and heights above it by **0.28**.
>
> **Bedrock and deepslate are probability ramps, not hard cutoffs, and this document said otherwise until 2026-08-19.** It claimed "bedrock a hard slab at y ≤ 2, deepslate a hard cutoff at y ≤ 12"; **both the numbers and the mechanism were wrong**, read out of [game/src/world/TerrainGenerator.cpp](game/src/world/TerrainGenerator.cpp) at the `kBedrockSolid` / `kDeepslateAlways` constants and their two use sites in the downward classification walk. Bedrock is **solid at y ≤ `kBedrockSolid` = 0**, then a **per-block dice roll** rising through `y ≤ kBedrockTop = 4` at probability `(kBedrockTop + 1 − y) / (kBedrockTop + 1)` — so y 1 is 80% bedrock, y 4 is 20%, and the familiar uneven floor is a gradient rather than a slab. Deepslate is **always at y ≤ `kDeepslateAlways` = 9**, then rolls with probability `(kDeepslateNever − y) / (kDeepslateNever − kDeepslateAlways)` up to `kDeepslateNever = 15`, above which it never appears. This matters to anyone designing an ore band or a floor: **there is no single y at which either changes**, and code that tests `y <= 12` for deepslate is testing a number that appears nowhere in the generator.
>
> **Lava.** New on **2026-08-19**: a flood layer fills carver-emptied cells at or below **`kLavaLevel` = 6**, applied after the downward classification walk and before the ocean fill, with `static_assert`s pinning it above `kBedrockTop` and below `kSeaLevel`. **Source class: secondary — mapped, not ported.** `Mojang/bedrock-samples` publishes no worldgen data, so the constant was *derived* from the reference's aquifer lava rule by two independent routes that both round to 6, rather than read off a published value. **Do not cite it as a reference constant**; it is our best mapping of one, dated, and it is the number that would change first if the world height ever changes.
>
> **Caves** are **three carvers and an entrance mask**, not one formula — `isCave` in [game/src/world/TerrainGenerator.cpp](game/src/world/TerrainGenerator.cpp), and the reference's own cheese/spaghetti/noodle split is now ported rather than merely admired. Bedrock at y ≤ 4 is never carved, so there is always something to stand on. **Spaghetti**, the tunnels: `|octaves3D| < 0.060 × fade` at a 58-block wavelength with Y stretched by 1/0.62. **Cheese**, the caverns: `octaves3D > 0.44 + (1 − fade)` at 104 blocks, Y 1/0.72. **Noodle**, the thin worms: `|octaves3D| < 0.032 × fade` at 27 blocks and isotropic, confined below y = 34 and gated behind a 2D patch field at 190 blocks ≥ 0.10 — the cheap 2D test first, so the third 3D sample stays off the common path entirely. It is worth naming what the two forms are: **a band around zero (`|x| < w`) is the spaghetti trick and a one-sided threshold is the cheese trick**, and this generator now uses each where it belongs — the zero-crossing shell gives connected winding tunnels, while thresholding the field directly gives the disconnected blobs that read as caverns. **Only spaghetti is ever allowed near daylight**, and only inside a mouth: a separate `octaves2D` entrance field at a 230-block wavelength, thresholded at 0.26 over a 0.14-wide ramp and forced to zero on any column whose surface is below sea level, relaxes the tunnel carver's 6-block surface margin and its 10-block fade to nothing. Caverns and noodles keep a full 12-block margin whatever the entrance says — a cavern that surfaces is not a cave mouth, it is a crater the size of the cavern with the trees that were standing on it left in mid-air. The lesson we already paid for — **cave cost tracks surface area, not hollow volume**, so halving the width barely moved the triangle count while halving the frequency did — is exactly why the reference separates the three: three different surface-area budgets.
>
> **Every number in the two paragraphs above is a WAVELENGTH in blocks per cycle, never a frequency.** The code always divides — `worldX / kPatchWavelength` — so a larger number means a *larger, smoother* feature. The paragraphs these replaced quoted **frequencies**, and the two are **reciprocals**: writing 84 where a reader expects a frequency describes a field about 7,000× wrong, and nothing in a build would object. The full set, all in blocks per cycle: base XZ **84**, base Y **210**, patch **64**, spaghetti **58**, cheese **104**, noodle **27**, noodle patch **190**, entrance **230**, cover patch **42**, surface depth **12**.
>
> **"Three carvers and an entrance mask" is deliberate, and a finding asking for "four carvers" was refused on 2026-08-19.** Finding 9689 described `isCave` as the disjunction of four carvers with entrance as the fourth. It is not. Read at `isCave`, the entrance field produces an `opening` scalar that **scales the spaghetti carver's surface margin and fade toward zero** — it never returns air on its own, and caverns and noodles ignore it entirely and keep the full `kCavernSurfaceMargin`. A reader who believed the four-carver framing would look for an air test that does not exist, or add a surface margin to a field that is itself a margin. The distinction is the same one the code's own comment makes: *only the tunnel carver is ever allowed near daylight.*
>
> **Ores.** **Nine** generate (the eight metals plus ancient debris), from `kOreVeins` in [game/src/world/TerrainGenerator.cpp](game/src/world/TerrainGenerator.cpp) ordered **rarest first**, so the first match wins and a common ore can never overwrite a scarce one where their bands overlap. *(§15.2 of this file previously said eight; the code says nine. The code wins.)* Two deliberate simplifications. Ours are **thresholded 3D value noise** rather than trapezoid-distributed vein placement — same mechanism as the caves at a much higher frequency, which gives connected blobs for free where a per-cell roll would read as speckle. And the bands are compressed onto our world by the 0.17/0.28 factors above. Each threshold comes from the reference's share of rock via `t = 1 − sqrt(share)`, which holds because one octave of value noise is near enough triangular. Ancient debris is **not** kept away from air the way the reference's buried ores are — that rule exists to stop it being spotted from a lava lake. **That rationale expired on 2026-08-19: lava now generates (see below), so the reason to skip the rule is gone and the rule is simply absent.** *(Before that date this sentence read "and we have no lava", which was true when written and false the moment `kLavaLevel` landed — the exact shape this document exists to catch.)*
>
...
- **The 7D box search with an R-tree.** At **thirty** biomes a linear scan over `kBiomes` is still the right algorithm, and it already runs at full dimensionality — first match wins, so the average scan is shorter than the row count. Revisit at ~40 rows, not before. *(This bullet said "seven biomes" until 2026-08-19; the table has grown past four times that and the advice still holds, but it is now within sight of its own trigger.)*
...
- **The badlands 192-entry clay-band table.** A lot of machinery for one biome we do not have.
  ⛔ **Correction, 2026-08-17.** The badlands *is* a biome now (`world/Biome.cpp`, 27 of them, and
  the linear scan below is at 27 not 7). The table is still not worth taking, but the reason above
  is no longer the reason — it is machinery for one biome's *stripes*, not for the biome itself.
...
> ✅ **INFO ALREADY USED — every row on this list is now closed.** The nine already struck through
> were done by 2026-08-06; of the five that were still live, **6** (the invulnerability window)
> landed at M21, **12** (continentalness and erosion) at M20p, **13** (the water weight search) at
> M20h, and **14** (flying and swimming in one milestone) at M20b/M20i. Only **11** (ingredient
> tags) was never built, and it was superseded rather than dropped — see the correction at §11.1.
> Kept as a record of what the order turned out to be, not as work.
...
> ⚠ **USED — BUT SEE `audit.md` §13.1 and §13.2.** The bow, the arrow and the archer behaviour were
> all built from this section (2026-08-08, corrected at M29l). Two faults are open and both are
> severe: **`Projectile.cpp:157` has no player in it at all, so a skeleton's arrow can never hit
> you**, and **a mob's arrow that hits another creature is blamed on you**, which turns your wolves
> hostile. **§17.8, the drowned's trident, is `ACTIVE` and unbuilt.**
...
## 17.6 How a mob decides to shoot — `minecraft:behavior.ranged_attack`

Researched 2026-08-11 against `Mojang/bedrock-samples` `behavior_pack/entities/`. §17.1 is about the arrow in flight; this is about the mob that let it go.

**Every timing field in this component is in SECONDS.** `attack_interval`, `attack_interval_min`, `attack_interval_max`, `burst_interval`, `charge_charged_trigger`, `charge_shoot_trigger` and `target_in_sight_time` are all documented "in seconds"; **nothing in it is in ticks.** That is worth stating loudly, because §17.1 immediately above is *entirely* per-tick and mixing the two is this project's third-most-expensive bug shape. `attack_radius` is in blocks; `speed_multiplier` and `chance` are unitless.

| Mob | Reload, Normal | Reload, Hard | Radius | Notes |
|---|---|---|---|---|
| **Skeleton** | **3.0 s** | 2.0 s | **15** | `attack_interval_min` = `max`, so no randomisation |
| **Stray** | 3.0 s | 2.0 s | 15 | `aux_val: 19` on the shooter — a **slowness**-tipped arrow |
| **Bogged** | **3.5 s** | 2.5 s | 15 | uses the bare scalar `attack_interval`, not the min/max pair; `aux_val: 26` — **poison** |
| **Drowned** | **1.0–3.0 s** | — | **10** | the only one with min ≠ max, and it **scales with target distance**; `swing: true` |
| **Witch** | 3.0 s | — | 10 | `speed_multiplier: 1`, priority 2 |
| **Pillager** | 1.0 s | — | 8 | ranged by default |
| **Piglin** | 1.0 s | — | 8 | the only one with `attack_radius_min` (**4**) and `target_in_sight_time` (0.1 s) |

**Ours is `kArcherInterval = 1.0f`, which is three times too fast**, and `kArcherRange = 15.0f`, which is exactly right. The piglin's `attack_radius_min` of 4 is independent confirmation of our `kArcherTooClose = 4.0f`.

### ⚠ There is no bow wind-up in the data, and looking for one is a trap

- **`charge_shoot_trigger` is absent from every one of the seven mobs above.** Only the blaze and the llama use the charged-burst regime, and the schema makes the two regimes **mutually exclusive** rather than concurrent — `attack_interval*` is explicitly qualified "when *not* using a charged shot". So for an archer there is exactly one clock.
- **The crossbow users draw with a different component**, `minecraft:behavior.charge_held_item` (pillager and piglin, priority 3 so it outranks the shot), and **that component carries no duration field at all.** The draw length is engine-side.
- **The skeleton does not have that component either**, and its bow pose is gated on `query.has_target` alone (`ANIMATION.md` §7.4). **A skeleton's bow is simply always drawn while it has a target**; the 3.0 s reload happens behind it.

**So a wind-up is ours to invent, and must be commented as ours.** What the data supports is a raised bow the whole time a target is held, with the shot at the end of each interval.

## 17.7 The witch, which is far more specified than the skeleton

Both of the witch's systems live in the behaviour pack rather than in engine code, which makes it the most faithfully reproducible mob on the roster.

**Which potion it throws** is an ordered `projectiles` list on `minecraft:shooter`, first match wins, defaulting to **harming**. `power: 0.75`, `magic: true`, `sound: "throw"`.

| Order | Potion | Condition |
|---|---|---|
| 1 | Healing (21) | target is a raider **and** its health ≤ 4 · `lose_target: true` |
| 2 | Regeneration (28) | target is any raider · `lose_target: true` |
| 3 | Slowness (17) | target **≥ 8 blocks** away and not already slowed |
| 4 | Poison (25) | target health **≥ 8** and not already poisoned |
| 5 | Weakness (34) | target **≤ 3 blocks** away, not already weakened, **25% chance** |
| — | **Harming (23)** | when nothing above matches |

**We have no raiders, so rows 1 and 2 are dead for us** and the live rule is a clean three: slowness beyond 8 m, weakness inside 3 m at one chance in four, poison against anything on 8 health or more, harming otherwise.

**It drinks on itself** — `minecraft:behavior.drink_potion`, **priority 1**, so drinking outranks throwing:

| Potion | chance | trigger |
|---|---|---|
| Water Breathing (19) | 0.15 | self underwater, not already water-breathing |
| Fire Resistance (12) | 0.15 | on fire **or** on a hot block **or** taking fire damage, and not already resistant |
| Healing (21) | 0.05 | self missing health |
| Swiftness (14) | 0.25 | has a target, not already sped up, **target ≥ 11 blocks away** |

`speed_modifier: -0.25` — **it moves a quarter slower while drinking**. And `magic: true` on the shooter is the interlock the schema spells out: the ranged-attack goal **will not run at the same time as `drink_potion`**. A witch never throws and drinks in the same moment, which is what makes the drink read as a decision rather than a stutter.

⚠ **Not established by the data: how long a drink lasts.** There is no `drink_delay` and no duration field anywhere — `drink_potion` has only `control_flags`, `potions`, `priority` and `speed_modifier`. Nor is the cadence at which `chance` is rolled. Both are engine-side, so **both are ours to pick and must be commented as ours.**

## 17.8 The drowned's trident, gated three ways

> **ACTIVE — not built.** The drowned walks the seabed and the `Trident` item exists in the
> catalogue, but nothing throws one and there is no ranged/melee split by distance. All three gates
> below are unspent.

```json
"minecraft:behavior.ranged_attack": {
  "attack_interval_max": 3.0, "attack_interval_min": 1.0,
  "attack_radius": 10.0, "priority": 3, "swing": true }
"minecraft:shooter": { "def": "minecraft:thrown_trident", "sound": "item.trident.throw" }
```

1. **Only 6.25% of drowned spawn ranged at all** — a nested `randomize` weighting 25 against 375 inside every branch — and a ranged one is given `minecraft:ranged_equipment` instead of `melee_equipment`.
2. **It switches back to melee by distance, with hysteresis**, through `minecraft:target_nearby_sensor`: **inside 3.0 blocks → melee, outside 5.0 → ranged**, and between the two it keeps whatever mode it was in. The two goals never coexist because the component groups are mutually exclusive, so this is not a priority tie-break.
3. **`swing: true`** drives `variable.attack_time` — so unlike a skeleton, a drowned's throw *does* swing the arm. `ANIMATION.md` §7.4 has what that flag animates.

---

# 18. Thrown items: the pearl and the egg

> ✅ **INFO ALREADY USED.** The void pearl shipped at M20y as a real arcing throw, and the pearl's
> inertia of exactly 1 — the fact that decides its 45-block ceiling — is pinned by a `static_assert`.
> Kept for reference. Our three named divergences are recorded in §18.3.
...
> ⛔ **Correction, 2026-08-17.** "there is no health yet" is stale — health and fall damage both
> shipped at M21 (`world/Survival.hpp`). The divergence itself may still stand; what is wrong is the
> reason given for it. Check `world/Projectile.cpp` before assuming either way.

---

## 19. Villages, villagers and the iron golem

> ⚠ **USED — BUT SEE `audit.md` §13.3.** M29d built the lot: the grid, five village types off the
> biome table, fifty-five buildings, the claim scan, the five-window schedule, fourteen trades and
> the golem. The audit found that **a villager's bed, job and bell claim is never saved**, so a
> village stops working after a reload; §13.10 and §13.11 are two smaller faults in the same code.

Researched 2026-08-10 by five parallel agents against `Mojang/bedrock-samples`.
The shipped implementation is `SYSTEM_MEMORY.md` → "Villages" and "A villager's
day"; what follows is only what the reference does and the handful of facts that
are not obvious from playing it.

**Bedrock ships no `worldgen/` data at all.** The behaviour pack has `biomes`,
`entities`, `items`, `loot_tables`, `recipes`, `shapes`, `spawn_rules` and
`trading`, and nothing else — village structures are engine C++ with the data
baked into the client. So every piece-pool number in the world is Java's, and is
marked `[JE]` wherever it is quoted. What *is* Bedrock's, and is the more useful
half, is the biome mapping and the entity behaviour.

### 19.1 Placement

- **`minecraft:village_type` is a component on the biome**, added in Bedrock
  26.0, and its schema permits exactly five values: `desert`, `ice`, `savanna`,
  `taiga`, `default`. **A biome without the component never generates a
  village.** Ten biomes carry it — plains, sunflower plains and meadow at
  `default`; desert; savanna; taiga, taiga hills, snowy taiga and snowy taiga
  hills; and snowy plains at `ice`. Bedrock is more generous than Java here,
  which the wiki names as the reason a Bedrock world is likelier to have a
  village near spawn.
- **The grid.** `random_spread` with `spacing`, `separation`, `salt` and
  `spread_type`; a region is `floorDiv(chunk, spacing)`, the offset is
  `nextInt(spacing - separation)` on each axis, and the RNG is seeded
  `regionX * 341873128712 + regionZ * 132897987541 + worldSeed + salt`. Java's
  village set is `spacing 34, separation 8, salt 10387312` **in chunks**;
  Bedrock's own numbers are not published.
- **Separation is a floor on regularity, not a promise of no overlap.** Two
  adjacent cells can put their origins 9 chunks apart while a village's
  `max_distance_from_center` is 80 blocks, so villages genuinely can merge. The
  `villages` structure set has no `exclusion_zone` at all — only pillager
  outposts avoid anything, and that is one-way.
- **There is no flatness test.** Villages are placed on whatever the heightmap
  says and the *terrain* is bent to suit, via `terrain_adaptation: beard_thin`
  — a density-function modifier that raises ground under the structure and
  carves rock out of it before the surface rules run. Desert pyramids and
  mansions do get a terrain check; villages pointedly do not.
- **One biome sample decides the type for the whole village**, taken at the
  start position before any piece is placed. A village that starts in desert and
  sprawls into badlands stays a desert village. **Villager skins, by contrast,
  follow the biome each villager spawns in** — which is the entire reason jungle
  and swamp villager outfits exist, since neither biome ever hosts a village.

### 19.2 Assembly `[JE]`

- Jigsaw, depth `size: 6`, `max_distance_from_center: 80`, start pool
  `village/<type>/town_centers`, breadth-first with a stable priority queue. A
  piece is rejected only for leaving the outer box or intersecting one already
  placed.
- **`rigid` versus `terrain_matching` is per pool element**, and the split is
  total: every street and terminator is terrain-matching, every town centre,
  house, decor, animal and villager marker is rigid. Terrain matching slides the
  whole piece onto the heightmap at placement *and* drops each column
  individually at processing.
- Street pools outweigh junction pieces about 2:1 in every type, which is what
  makes a village read as roads rather than as a maze. `empty_pool_element` at
  roughly a tenth of the house pool is the density knob.
- **Road processors are one line each and are most of what sells a street**:
  dirt path over water becomes the type's planks, and 10% (plains) or 20%
  (savanna, taiga, snowy) of path cells become grass. Desert has no processor —
  its streets are smooth sandstone as authored.
- **The support platform is circular in Bedrock and square in Java.** It is the
  backstop under a building that would otherwise overhang.
- **Bedrock plants fully-grown crops where Java plants seeds.** Farm processors
  pick wheat, with 30% carrots / 20% potatoes / 10% beetroot in plains.

### 19.3 Villagers

- **Thirteen trades, thirteen job blocks**: composter, barrel, fletching table,
  loom, cartography table, lectern, stonecutter, smithing table, grindstone,
  blast furnace, smoker, cauldron, brewing stand. The villager's own
  `minecraft:preferred_path` cost-50 list is exactly those thirteen plus the
  bed, which corroborates the set from Bedrock's own data.
- **Claiming is village-wide, not per villager.** `minecraft:dweller` scans 16
  blocks horizontally and 4 vertically every `60 ± 40` ticks and adds what it
  finds to a shared list; an unemployed villager **that already has a bed**
  takes the first entry. `[JE]` Java uses a 48-block sphere, provisional claims
  and a 60-second reachability timeout — do not build that.
- **The schedule is a `minecraft:scheduler` component** with five windows out of
  24000 ticks (0–8000 work, –10000 gather, –11000 work, –12000 home, –24000
  bed) and a per-villager `max_delay_secs: 10` stagger. `behavior.work` is
  `active_time: 250`, `goal_cooldown: 200`, half speed, and its `on_arrival`
  fires the trade restock — which is why restocking is "twice a day" without any
  counter anywhere.
- **"Go home" is the absence of anything better to do.** The home phase's
  component group is literally `{}`; the always-present `behavior.move_indoors`
  at priority 6 stops being outranked once work and mingle are removed.
- **A generated village spawns nobody employed**: 5% baby, 95% adult, and of the
  adults 90% unskilled and 10% nitwit. A **spawn egg** is different — it rolls
  uniformly over all thirteen trades plus nitwit.
- **Villagers do not spawn naturally at all in Bedrock.** There is no
  `spawn_rules/villager*.json`. Village generation, breeding, curing, an igloo
  basement or a spawn egg are the only sources.
- Panic is flat: one `avoid_mob_type` entry, **8 blocks for all five hostile
  families**, at 0.6× speed. Java's graded 8/10/12/15 table is Java-only.
- Zombie conversion: **0% easy, 50% normal, 100% hard**. Curing is a golden
  apple plus Weakness, and the shipped `transformation.delay` is **100 seconds**
  bounded 80–200 — the wiki's "five minutes" is Java's number leaking into a
  shared paragraph.

### 19.4 The iron golem

- **There is no `spawn_rules/iron_golem.json` and no `minecraft:angry`
  component.** Both were checked twice. Village spawning is engine code, and the
  golem's temperament is structural: a player-built one gets a `hurt_by_target`
  row that filters the player out, so it is passive to you for ever with no
  timer to expire.
- **Creepers are excluded from all four targeting rows** — `hurt_by_target`,
  `target_when_pushed`, `nearest_attackable_target` and (by omission)
  `defend_village_target`.
- Health 100, collision box 1.4 × 2.9, movement 0.25, knockback resistance 1.0,
  no armour component, immune to fall damage, no `breathable` so it does not
  drown, follow range 64, never despawns.
- `minecraft:attack.damage` is a **range, 7–21**. `minecraft:apply_knockback_rules`
  is `horizontal_power 0.52`, `vertical_power 0.39`, capped at 0.8 — and the
  player's own entry is 0.165 / 0.105, so the transferable figures are **3.15×
  and 3.71×**, never the raw numbers.
- Bedrock village rule: one golem when a village first generates, then **≥ 10
  villagers and ≥ 20 beds** for more, one attempt per 700 ticks in a 17×13×17
  box. Java instead has villagers summon them while gossiping or panicking.
- Drops **3–5 iron ingots and 0–2 poppies**, no looting modifier, no XP.
- Cracking is a **second render controller over the same geometry**, indexed
  `clamp(floor(health / 25), 0, 3)` — the sheep-fleece arrangement, not four
  models. One iron ingot repairs exactly 25 health, which is exactly one crack
  level.
- `geometry.irongolem` is **128 × 128** with eight bones. Two pivots are not
  where a naive rig would put them: **both arms hinge at the body's centreline**
  `[0, 31, 0]`, not at their own tops, and the head hinges below itself at
  `[0, 31, -2]`. The 9×5×6 waist cube at uv (0, 70) with `inflate: 0.5` bridges
  a real five-texel gap between the legs and the chest — omitting it leaves a
  hole through the middle.
- `animation.iron_golem.attack` gives **both arms one expression with no sign
  flip**, heaving from −57° to −142.65° and back over ten ticks. It is a
  two-handed overhead swing, not a punch, and it is the mob's signature.
